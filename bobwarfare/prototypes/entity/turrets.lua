...
local function bob_gun_turret_extension(inputs)
  return {
    layers = {gun_turret_extension(inputs), {
...
local function bob_turret_base(inputs)
  ...
  if inputs.type == "gun" then
    anim = bob_gun_turret_base{
      tint = inputs.tint,
      size = size
    }
  elseif inputs.type == "laser" then
    anim = bob_laser_turret_base{
      tint = inputs.tint,
      size = size
    }
  else
    anim = bob_artillery_turret_base{
      tint = inputs.tint,
      size = size
    }
  end
...
local function bob_gun_turret(inputs)
  return {
    ...
    preparing_animation = bob_turret_extension{
      type = inputs.gun_type,
      tint = inputs.tint
    },
    folding_animation = bob_turret_extension{
      run_mode = "backward",
      type = inputs.gun_type,
      tint = inputs.tint
    },
    prepared_animation = bob_turret_attack{
      frame_count = 1,
      type = inputs.gun_type,
      tint = inputs.tint
    },
    attacking_animation = bob_turret_attack{
      type = inputs.gun_type,
      tint = inputs.tint
    },
...
local function bob_laser_turret(inputs)
  ...
  local turret = {
    ...
    folded_animation = bob_laser_turret_extension{
      frame_count = 1,
      line_length = 1,
      tint = inputs.tint,
      size = size
    },
    preparing_animation = bob_laser_turret_extension{
      tint = inputs.tint,
      size = size
    },
    folding_animation = bob_laser_turret_extension{
      run_mode = "backward",
      tint = inputs.tint,
      size = size
    },
    prepared_animation = bob_laser_turret_attack{
      tint = inputs.tint,
      size = size
    },
    base_picture = inputs.base,
...
data.raw["artillery-turret"]["artillery-turret"].fast_replaceable_group =
  "turret"

local turret =
  util.table.deepcopy(data.raw["artillery-turret"]["artillery-turret"])
