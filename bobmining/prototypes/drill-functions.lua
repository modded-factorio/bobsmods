data.raw.item["electric-mining-drill"].order =
  "a[items]-b-0[electric-mining-drill]"

local function set_electric_mining_drill_animation_speed(
object,
  animation_speed
)
...
function bobmods.mining.electric_mining_drill(inputs)
  local mining_drill =
    util.copy(data.raw["mining-drill"]["electric-mining-drill"])
  ...
  mining_drill.resource_searching_radius =
    inputs.resource_searching_radius or 2.49
  mining_drill.circuit_wire_max_distance =
    inputs.circuit_wire_max_distance or default_circuit_wire_max_distance

  mining_drill.energy_source.emissions_per_minute =
    inputs.emissions_per_minute or 10
  mining_drill.storage_slots = 6

  set_electric_mining_drill_animation_speed(
    mining_drill,
    inputs.animation_speed * 0.8
  )
