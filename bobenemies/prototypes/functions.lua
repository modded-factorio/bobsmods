...
function bobmods.enemies.new_biter(input)
  ...
  -- in pu
    ...
    collision_box = {
      {-0.4 * input.scale, -0.4 * input.scale},
      {0.4 * input.scale, 0.4 * input.scale}
    },
    selection_box = {
      {-0.7 * input.scale, -1.5 * input.scale},
      {0.7 * input.scale, 0.3 * input.scale}
    },
    sticker_box = {
      {-0.6 * input.scale, -0.8 * input.scale},
      {0.6 * input.scale, 0}
    },
...
function bobmods.enemies.spitter_attack_parameters(data)
  return {
    ...
    warmup = 30,
    projectile_creation_parameters = spitter_shoot_shiftings(
      data.scale,
