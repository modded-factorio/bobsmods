...
announce_character_swap.minime = function(data)
  wlog(
    "Entered function announce_character_swap.minime(" .. serpent.line(
      data
    ) .. ")"
  )
...
announce_character_swap.jetpack = function(data)
  wlog(
    "Entered function announce_character_swap.jetpack(" .. serpent.line(
      data
    ) .. ")"
  )
...
local function announce(mod_name, data)
  wlog(
    "Entered function announce(" .. tostring(mod_name) .. ", " .. serpent.line(
      data
    ) .. ")"
  )
  ...
  if mod_name and announce_character_swap[mod_name] then
    ...
    error(
      "No such function: announce_character_swap[" .. tostring(mod_name) .. "]!"
    )
  end
end
...
function gui_add_title(gui, title, button_name, drag_target)
  wlog(
    "Entered function gui_add_title(" .. serpent.line(
      gui
    ) .. ", " .. serpent.line(title) .. ", " .. serpent.line(
      button_name
...
remote.add_interface("bobclasses", {
  ...
  get_classes = function()
    ...
    wlog(
      "next(global.classes): " .. tostring(
        global.classes and next(global.classes) and true or false
      )
    )
  ...
  on_character_swapped = function(event)
    ...
    local player =
      (new and new.valid and new.player or new.associated_player) or (old and old.valid and old.player or old.associated_player)
    if player then
...
function add_class(name, class)
  wlog(
    "Entered function add_class(" .. name .. ", " .. serpent.line(class) .. ")"
  )
...
function add_classes()
  ...
  remote.call("bobclasses", "add_classes", {
    builder = {
      ...
      bonuses = {character_crafting_speed_modifier = 1},
      starting_inventory = {
        add = {{
...
function create_button(player_index)
  ...
  if mod_gui.get_button_flow(
    game.players[player_index]
  ).bob_avatar_toggle_gui then
    mod_gui.get_button_flow(
      game.players[player_index]
    ).bob_avatar_toggle_gui.destroy()
  end
...
function minime_exchanged_characters(event)
  wlog(
    "Entered function minime_exchanged_characters(" .. serpent.line(
      event
    ) .. ")"
  )
  ...
  elseif editor then
    wlog(
      "Calling switch_to_editor(" .. player_index .. ", " .. tostring(
        old_character and old_character.name
      ) .. ")!"
    )
  ...
  elseif god then
    wlog(
      "Calling switch_to_god(" .. player_index .. ", " .. tostring(
        old_character and old_character.name
      ) .. ")!"
    )
...
function get_minime_events()
  ...
  return (remote.interfaces["minime"] and remote.interfaces["minime"].get_minime_event_ids and remote.call(
    "minime",
    "get_minime_event_ids"
  )) or {}
end
...
function register_minime_events()
  ...
  if minime_events then
    ...
    if minime_events["minime_exchanged_characters"] then
      wlog(
        'Registering handler for custom event "minime_exchanged_characters"!'
      )
      script.on_event(
        minime_events["minime_exchanged_characters"],
...
function init()
  ...
  add_class("builder-2", {
    ...
    bonuses = {character_crafting_speed_modifier = 2},
    starting_inventory = {
      add = {},
      replace = {{
        remove = {
...
script.on_event(defines.events.on_player_respawned, function(event)
  wlog(
    "Entered event handler on_player_respawned(" .. serpent.line(event) .. ")"
  )
  --player_index :: uint
...
script.on_event(defines.events.on_pre_player_died, function(event)
  wlog(
    "Entered event handler on_pre_player_died(" .. serpent.line(event) .. ")"
  )
  ...
  function(event)
    ...
    wlog(
      "entity.type: " .. tostring(
        event.entity.type
      ) .. "\tentity.name: " .. tostring(event.entity.name)
    )
...
script.on_event(defines.events.on_runtime_mod_setting_changed, function(event)
  wlog(
    "Entered event handler on_runtime_mod_setting_changed(" .. serpent.line(
      event
    ) .. ")"
  )

  if event.setting_type == "runtime-global" then
    if event.setting == "bobmods-classes-god-mode" or event.setting == "bobmods-classes-editor-mode" or event.setting == "bobmods-classes-class-select" or event.setting == "bobmods-classes-class-select-user" then
      for i, player in pairs(game.players) do
...
script.on_event(defines.events.on_research_finished, function(event)
  wlog(
    "Entered event handler on_research_finished(" .. serpent.line(event) .. ")"
  )
...
script.on_event(defines.events.on_gui_click, function(event)
  ...
  wlog(
    "Name of clicked button: " .. tostring(
      event.element.valid and event.element.name
    )
  )
  ...
  if event.element.valid and string.find(
    event.element.name,
    "bob_avatar_list_character_"
  ) then
    local i = tonumber(string.match(event.element.name, "%d+"))
    local characters =
      game.players[event.player_index].get_associated_characters()
  ...
  if event.element.valid and event.element.name == "bob_avatar_rename" then
    global.players[event.player_index].rename_mode =
      not global.players[event.player_index].rename_mode
    refresh_avatar_gui(event.player_index)
...
script.on_event(defines.events.on_gui_text_changed, function(event)
  wlog(
    "Entered event handler on_gui_text_changed(" .. serpent.line(event) .. ")"
  )
  if event.element.valid and string.find(
    event.element.name,
    "bob_avatar_list_textfield_"
  ) then
    local characters =
      game.players[event.player_index].get_associated_characters()
    local i = tonumber(string.match(event.element.name, "%d+"))
...
function try_to_reopen_class_select(event)
  wlog(
    "Entered function try_to_reopen_class_select(" .. serpent.line(event) .. ")"
  )
  local player = game.players[event.player_index]
...
function add_class_gui_classes(gui, player_index)
  wlog(
    "Entered function add_class_gui_classes(" .. serpent.line(
      gui
    ) .. ", " .. player_index .. ")"
  )
  local player = game.players[player_index]
...
function draw_avatar_gui(player_index)
  ...
  globtable.current_character = gui.add({
    type = "flow",
    name = "current_character_info"
  })
  ...
  if gui then
    ...
    if player.character then
      ...
      gui.add({
        type = "label",
        name = "current",
        caption = {"gui.bob-current-character"}
      })
      gui.add{
        type = "sprite-button",
      ...
      if global.players[player_index].rename_mode then
        ...
        gui.add({
          type = "textfield",
          name = "bob_avatar_current_character_textfield"
        })
        if global.names[entity.unit_number] then
          gui["bob_avatar_current_character_textfield"].text =
            global.names[entity.unit_number]
        end
      else
        gui.add({
          type = "label",
        ...
        if global.names[entity.unit_number] then
          gui["bob_avatar_current_character_name"].caption =
            global.names[entity.unit_number]
        end
      end
    elseif player.controller_type ~= defines.controllers.character then
      gui.add({
        type = "label",
        name = "bob_avatar_current_character_name",
        caption = {"gui.no-character"}
      })
      if player.controller_type == defines.controllers.god then
        gui.bob_avatar_current_character_name.caption =
          {"gui.bob-avatar-god-mode"}
      elseif player.controller_type == defines.controllers.editor then
        gui.bob_avatar_current_character_name.caption =
          {"gui.bob-avatar-editor-mode"}
      end
    end
  end
end
...
function draw_characters_list(player_index)
  ...
  if gui then
    ...
    if table_size(characters) == 0 then
      gui.add({
        type = "label",
        name = "bob_avatar_no_list",
        caption = {"gui.no-avatars"}
      })
      return
    end
    ...
    for i, entity in pairs(characters) do
      ...
      if global.players[player_index].rename_mode then
        ...
        gui.table.add({
          type = "textfield",
          name = "bob_avatar_list_textfield_" .. i
        })
        if global.names[entity.unit_number] then
          gui.table["bob_avatar_list_textfield_" .. i].text =
            global.names[entity.unit_number]
        end
      else
        gui.table.add({
          type = "label",
          name = "bob_avatar_list_name_" .. i,
          caption = entity.prototype.localised_name
        })
        if global.names[entity.unit_number] then
          gui.table["bob_avatar_list_name_" .. i].caption =
            global.names[entity.unit_number]
        end
      end
      gui.table.add{
      ...
      if entity.unit_number == global.players[player_index].minimap_unit then
        gui.table["bob_avatar_list_minimap_" .. i].style =
          "selected_mod_gui_button_28"
      end
    end
  end
end
...
function draw_minimap_gui(player_index, entity)
  ...
  if global.players[player_index].minimap_unit == entity.unit_number then
    ...
    if not gui and global.players[player_index].gui and global.players[player_index].gui.main_flow then
      gui = global.players[player_index].gui.main_flow.add{
        type = "flow",
        name = "minimap_flow"
      }
      gui.style.padding = 4
...
function set_character_icon(player_index, entity)
  wlog(
    "Entered function set_character_icon(" .. player_index .. ", " .. entity.name")!"
  )
...
function create_character(player_index, class)
  wlog(
    "Entered function create_character(" .. player_index .. ", " .. serpent.line(
      class
    ) .. ")!"
  )
  local player = game.players[player_index]
  ...
  if player.controller_type == defines.controllers.character and player.character ~= nil and player.character.valid then
    ...
    wlog(
      "Old character: " .. tostring(old and old.name) .. " (" .. tostring(
        old and old.unit_number
      ) .. ")"
    )
    wlog(
      tostring(old and old.name) .. " ~= " .. tostring(
        class and class.entity_name
    ...
    if old and old.valid and old.name ~= class.entity_name then
      ...
      if remote.interfaces["minime"] and remote.interfaces["minime"].bob_class_pick_character then
        skin =
          remote.call("minime", "bob_class_pick_character", player_index, class)
      end
      wlog(
        "Old entity still exists after call to minime: " .. tostring(
          old and old.valid
        )
      )
      ...
      wlog(
        "Old entity still exists after setting skin: " .. tostring(
          old and old.valid
        )
      )
      ...
      wlog(
        "Old entity still exists after creating new character: " .. tostring(
          old and old.valid
        )
      )
      ...
      wlog(
        "Old entity still exists after announcing new character: " .. tostring(
          old and old.valid
        )
      )
      ...
      wlog(
        "Old entity still exists after setting direction: " .. tostring(
          old and old.valid
        )
      )
      ...
      if global.players[player_index].first_character then
        ...
        if class.starting_inventory then
          ...
          for i, item in pairs(class.starting_inventory.replace) do
            if item.remove and item.add and player.get_item_count(
              item.remove.name
            ) >= item.remove.count then
...
function switch_character(player_index, new_character)
  wlog(
    "Entered function switch_character(" .. player_index .. ", " .. tostring(
      new_character and new_character.name
    ) .. ")!"
  )
  ...
  if remote.interfaces["jetpack"] then
    ...
    local jetpacks =
      remote.interfaces["jetpack"].get_jetpacks and remote.call(
        "jetpack",
        "get_jetpacks",
        {}
      )
    ...
    if old_unit_number and jetpacks[old_unit_number] and remote.interfaces["jetpack"].block_jetpack then
      wlog(
        "Old character was flying. Must block jetpack of character " .. old_unit_number
      )
    ...
    if new_unit_number and jetpacks[new_unit_number] and remote.interfaces["jetpack"].unblock_jetpack then
      wlog(
        "New character is flying. Must unblock jetpack of character " .. new_unit_number
      )
...
function switch_to_editor(player_index, old_char)
  wlog(
    "Entered function switch_to_editor(" .. player_index .. ", " .. tostring(
      old_char and old_char.name
    ) .. ")"
  )
  wlog(
    string.format(
      "Controller of player %s: %s",
      player_index,
