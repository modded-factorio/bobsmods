if settings.startup["bobmods-assembly-oilfurnaces"].value == true then
  ...
  if settings.startup["bobmods-assembly-limits"].value == true then
    if data.raw.recipe["fluid-mixing-furnace"] then
      data.raw["assembling-machine"]["fluid-mixing-furnace"].ingredient_count =
        4
    end
    if data.raw.recipe["fluid-chemical-furnace"] then
      data.raw["assembling-machine"]["fluid-chemical-furnace"].ingredient_count
      = 4
    end
  end
  ...
  if settings.startup["bobmods-plates-convert-recipes"] and settings.startup["bobmods-plates-convert-recipes"].value == true then
    if data.raw.item["steel-pipe"] then
      bobmods.lib.recipe.remove_result(
        "steel-furnace-from-fluid-furnace",
        "pipe"
      )
      bobmods.lib.recipe.add_result("steel-furnace-from-fluid-furnace", {
        "steel-pipe",
        2
      })
    end
  ...
  if mods["space-exploration"] then
    for _, furnace in pairs({"fluid-furnace"}) do
