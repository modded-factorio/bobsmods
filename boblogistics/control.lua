...
function bobmods.logistics.create_global_table_basic()
  ...
  if not global.bobmods.logistics.blacklist then global.bobmods.logistics.blacklist = {} end
end
...
function bobmods.logistics.set_positions(entity, player_index)
  ...
  if enabled then
    ...
    local new_positions = {
      drop_position = {x = entity.position.x + drop_position.x + drop_offset.x, y = entity.position.y + drop_position.y + drop_offset.y},
...
script.on_event(defines.events.on_built_entity, function(event)
  ...
  if settings.startup["bobmods-logistics-inserteroverhaul"].value == true and entity.type == "inserter" and entity.name == "long-handed-inserter" then
    entity.surface.create_entity{name = "red-inserter", position = entity.position, force = entity.force, direction = entity.direction, fast_replace = true, spill = false}
  end
end)
