...
bobmods.logistics.inserters.offset_positions = {
  near = {{
    x = 0,
    y = -bobmods.logistics.offset
  }, {
    x = bobmods.logistics.offset,
    y = 0
  }, {
    x = 0,
    y = bobmods.logistics.offset
...
function bobmods.logistics.create_gui_button(player_index)
  if not game.active_mods["bobinserters"] then
    ...
    local long_unlocked =
      tech_unlocked(player.force, bobmods.logistics.long_technology)
...
function bobmods.logistics.create_gui(player_index)
  ...
  local long_unlocked =
    tech_unlocked(
      ...
      bobmods.logistics.long_technology
    )
...
function bobmods.logistics.set_positions(entity, player_index)
  ...
  if enabled then
    ...
    local long_unlocked =
      tech_unlocked(force, bobmods.logistics.long_technology)
    ...
    local drop_position = {
      x = bobmods.math.round(drop_full_position.x),
      y = bobmods.math.round(drop_full_position.y)
    }
    ...
    local new_positions = {
      ...
      pickup_position = {
        x = entity.position.x + pickup_position.x,
        y = entity.position.y + pickup_position.y
      }
    }
...
script.on_event("bob-inserter-long", function(event)
  ...
  if not game.active_mods["bobinserters"] and entity and entity.type == "inserter" and player.can_reach_entity(
    entity
  ) and not global.bobmods.logistics.blacklist[entity.name] then
...
script.on_event(defines.events.on_built_entity, function(event)
  ...
  if not game.active_mods["bobinserters"] and event.item and event.item.place_result and event.item.place_result.type == "inserter" and (entity.type == "inserter" or (entity.type == "entity-ghost" and entity.ghost_type == "inserter")) and event.item.place_result.name == entity_name and not global.bobmods.logistics.blacklist[entity_name] then --probably don't even need this line anymore.
    bobmods.logistics.set_positions(entity, event.player_index)
  end

  if settings.startup["bobmods-logistics-inserteroverhaul"].value == true and entity.type == "inserter" and entity.name == "long-handed-inserter" then
