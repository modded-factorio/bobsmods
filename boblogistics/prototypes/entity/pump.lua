function bobmods.logistics.pump_height(
level --Regular Pumps are Height 4 which is 4x of a normal pipe => Higher tier pipes should have a height 4x higher then the pipe of the same tier
)
  return bobmods.logistics.pipe_height(level) * 4
end

function bobmods.logistics.pump_pumping_speed(
level --Pumping speed should match the capacity of the same tier pipe
)
  return bobmods.logistics.pipe_height(level) * 100
end

function bobmods.logistics.set_pump_height(
pump,
  level --Regular Pumps are Height 4 which is 4x of a normal pipe => Higher tier pipes should have a height 4x higher then the pipe of the same tier
)
  if data.raw["pump"][pump] then
    data.raw["pump"][pump].fluid_box.height =
      bobmods.logistics.pump_height(level)
  end
end

function bobmods.logistics.set_pumping_speed(
pump,
  level --Regular Pumps are Height 4 which is 4x of a normal pipe => Higher tier pipes should have a height 4x higher then the pipe of the same tier
)
  if data.raw["pump"][pump] then
    data.raw["pump"][pump].pumping_speed =
      bobmods.logistics.pump_pumping_speed(level)
  end
end
...
local fluid_wagon_connector_graphics =
  table.deepcopy(data.raw.pump.pump.fluid_wagon_connector_graphics)
...
data:extend({{
  ...
  animations = bobmods.logistics.pump_animations({
    r = 0.8,
    g = 0.8,
    b = 0.2,
    a = 1
