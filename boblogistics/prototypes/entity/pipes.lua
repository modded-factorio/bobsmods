function bobmods.logistics.pipe_distance(level)
  local range =
    ((level - 1) * settings.startup["bobmods-logistics-pipeperlevel"].value) + settings.startup["bobmods-logistics-pipestarting"].value
  return range
end
...
function bobmods.logistics.set_pipe_distance(pipe, level)
  if data.raw["pipe-to-ground"][pipe] then
    for index, connection in
      pairs(data.raw["pipe-to-ground"][pipe].fluid_box.pipe_connections)
    do
...
function bobmods.logistics.set_pipe_height(pipe, level)
  if data.raw["pipe"][pipe] then
    data.raw["pipe"][pipe].fluid_box.height =
      bobmods.logistics.pipe_height(level)
  end
  if data.raw["pipe-to-ground"][pipe] then
    data.raw["pipe-to-ground"][pipe].fluid_box.height =
      bobmods.logistics.pipe_height(level)
  end
end
...
data.raw["pipe-to-ground"]["pipe-to-ground"]["fluid_box"].pipe_covers =
  bob_pipecoverspictures("iron")
data.raw["pipe-to-ground"]["pipe-to-ground"].pictures =
  bob_pipetogroundpictures("iron")

data:extend({{ --2, --2, --0.5, --0.5, --2, --2,
  type = "pipe",
  name = "stone-pipe",
