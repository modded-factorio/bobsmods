...
local function generate_fill_recipe_icons(fluid, icon)
  ...
  elseif fluid.icons and util.combine_icons then
    icon = util.combine_icons(icon, fluid.icons, {
      scale = 0.5,
      shift = {4, -8}
    })
  end
...
function bobmods.lib.create_gas_bottle(fluid)
  if fluid and data.raw["item-subgroup"]["bob-empty-gas-bottle"] and data.raw["recipe-category"]["air-pump"] and data.raw["item-subgroup"]["bob-gas-bottle"] then
    ...
    local fill_recipe, empty_recipe =
      get_recipes_for_barrel(fluid.name .. "-barrel")
    ...
    if empty_recipe then
      ...
      bobmods.lib.recipe.add_result(empty_recipe.name, "gas-canister")
...
function bobmods.lib.create_fluid_canister(fluid)
  if fluid and data.raw["item-subgroup"]["bob-empty-canister"] and data.raw["recipe-category"]["barrelling"] and data.raw["item-subgroup"]["bob-canister"] then
    -- check if a barrel already exists for this fluid if not - create one
    local barrel_item = get_item(fluid.name .. "-barrel")
    if barrel_item then
