...
function bobmods.lib.resource.add_autoplace_peak(inputs)
  table.insert(
    data.raw.resource[inputs.resource].autoplace.peaks,
    bobmods.lib.resource.autoplace_peak(inputs)
  )
end
...
function bobmods.lib.resource.add_result(resource, item_in)
  ...
  if item and type(
    resource
  ) == "string" and data.raw.resource[resource] and bobmods.lib.item.get_type(
    item.name
  ) then
...
function bobmods.lib.resource.remove_result(resource, item)
  if type(resource) == "string" and type(
    item
  ) == "string" and data.raw.resource[resource] then
    bobmods.lib.result_check(data.raw.resource[resource].minable)
...
function bobmods.lib.resource.sprite(inputs)
  ...
  if inputs.filename then
    ...
    frame_count = 8
    variation_count = 8
    hr_version = bobmods.lib.resource.hr_sprite{
      sheet = 1,
      tint = inputs.tint
    }
...
function bobmods.lib.resource.effect(inputs)
  ...
  if inputs.filename then
    ...
    frame_count = 8
    variation_count = 8
    hr_version = bobmods.lib.resource.hr_effect{
      sheet = 5,
      tint = inputs.tint
    }
  end
...
function bobmods.lib.resource.create(inputs)
  ...
  data:extend({{
    ...
    stages = sheet,
    stage_counts = inputs.stage_counts or bobmods.lib.resource.stage_counts(
      sheet.sheet.variation_count,
      inputs.stage_mult or 100
    ),
  ...
  if inputs.infinite then
    ...
    data.raw.resource[inputs.name].infinite_depletion_amount =
  ...
  if inputs.icon then
    ...
    data.raw.resource[inputs.name].icons =
      bobmods.lib.resource.create_icon(inputs)
  end

  if inputs.required_fluid then
    data.raw.resource[inputs.name].minable.required_fluid =
      inputs.required_fluid
    data.raw.resource[inputs.name].minable.fluid_amount =
      inputs.fluid_amount or 10
  end

  if inputs.effect then
    local effect = inputs.effect
    effect.tint = inputs.tint
    data.raw.resource[inputs.name].stages_effect =
      bobmods.lib.resource.effect(effect)
    data.raw.resource[inputs.name].effect_animation_period =
      inputs.effect.animation_period or 5
    data.raw.resource[inputs.name].effect_animation_period_deviation =
      inputs.effect.animation_period_deviation or 1
    data.raw.resource[inputs.name].effect_darkness_multiplier =
      inputs.effect.darkness_multiplier or 3.6
    data.raw.resource[inputs.name].min_effect_alpha =
      inputs.effect.min_alpha or 0.2
    data.raw.resource[inputs.name].max_effect_alpha =
      inputs.effect.max_alpha or 0.3
  end
end

function bobmods.lib.resource.create_icon(inputs)
  local icon = {
    icon = "__boblibrary__/graphics/icons/ore-1.png",
    tint = inputs.tint,
    icon_size = 32
  }
  if inputs.sprite then
...
function bobmods.lib.resource.generate_data_stage(inputs)
  if inputs.name then
    if not inputs.particle then
      ...
      bobmods.lib.resource.create_particle{
        name = inputs.particle,
        tint = inputs.tint
      }
    end
