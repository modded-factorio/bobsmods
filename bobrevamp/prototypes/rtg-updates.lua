if bobmods.plates and settings.startup["bobmods-revamp-rtg"].value == true then
  ...
  if settings.startup["bobmods-revamp-hardmode"].value == true then
    ...
    bobmods.lib.recipe.add_result("sodium-cobaltate", {type = "fluid", name = "carbon-dioxide", amount = 150, catalyst_amount = 150})

    bobmods.lib.recipe.remove_result("sodium-carbonate", "water")
    bobmods.lib.recipe.add_result("sodium-carbonate", {type = "fluid", name = "pure-water", amount = 10, fluidbox_index = 1, catalyst_amount = 10})
