...
bobmods.inserters.offset_positions = {{
  x = -bobmods.inserters.offset,
  y = -bobmods.inserters.offset
}, {
  x = 0,
  y = -bobmods.inserters.offset
}, {
...
script.on_event("bob-inserter-pickup-rotate", function(event)
  ...
  if entity and entity.type == "inserter" and player.can_reach_entity(
    entity
  ) and not global.bobmods.inserters.blacklist[entity.name] then
...
script.on_event(defines.events.on_gui_opened, function(event)
  local player = game.players[event.player_index]
  if event.gui_type == defines.gui_type.entity and event.entity and event.entity.type == "inserter" and not global.bobmods.inserters.blacklist[event.entity.name] and settings.get_player_settings(
    player
  )["bobmods-inserters-gui-position"].value ~= "off" then
...
script.on_event(defines.events.on_gui_closed, function(event)
  ...
  (global.bobmods.inserters[event.player_index].position == "center" and event.gui_type == defines.gui_type.custom and event.element and event.element.valid and event.element.name == "bob_inserter_gui") then --the GUI actually exists.
    bobmods.inserters.delete_gui(event.player_index)
  end
end)
...
script.on_event(defines.events.on_gui_checked_state_changed, function(event)
  ...
  if event.element.valid then
    ...
    elseif event.element.name == "bob_logistics_inserter_long_out" then
      global.bobmods.logistics[event.player_index].long_out =
        event.element.state
    ...
    elseif event.element.name == "bob_logistics_inserter_enabled2" then
      global.bobmods.logistics[event.player_index].enabled2 =
        event.element.state
      if event.element.state then
    ...
    elseif string.sub(event.element.name, 1, 6) == "button" then
      ...
      if event.element.parent.name == "bob_logistics_pickup" then
        remote.call("bobinserters", "draw_button_position_check", {
          gui = event.element.parent,
          position = i
        })
      ...
      elseif event.element.parent.name == "bob_logistics_offset" then
        remote.call("bobinserters", "draw_button_offset_check", {
          gui = event.element.parent,
          position = i
        })
    ...
    if global and global.bobmods and global.bobmods.inserters then
      for i, player in pairs(game.connected_players) do
        if global.bobmods.inserters[player.index] then
          ...
          if entity and (not entity.valid or not player.can_reach_entity(
            entity
          )) then
            bobmods.inserters.delete_gui(player.index)
          end -- Create for this specific online player.
        else
          bobmods.inserters.create_global_table(player.index)
        end
      end -- Create for all if there are none.
    else
...
script.on_event(defines.events.on_research_finished, function(event)
  if event.research.name == bobmods.inserters.long_technology or event.research.name == bobmods.inserters.long2_technology or event.research.name == bobmods.inserters.near_technology or event.research.name == bobmods.inserters.more_technology or event.research.name == bobmods.inserters.more2_technology then
...
script.on_event(defines.events.on_built_entity, function(event)
  ...
  if event.item and event.item.place_result and event.item.place_result.type == "inserter" and (entity.type == "inserter" or (entity.type == "entity-ghost" and entity.ghost_type == "inserter")) and event.item.place_result.name == entity_name and not global.bobmods.inserters.blacklist[entity_name] then --probably don't even need this line anymore.
...
function split_drop_position(full_drop_position)
  local drop_position = {
    x = bobmods.math.round(full_drop_position.x),
    y = bobmods.math.round(full_drop_position.y)
  }
  local drop_offset = {
...
function combine_drop_position(drop_position, drop_offset)
  return {
    x = drop_position.x + drop_offset.x,
    y = drop_position.y + drop_offset.y
  }
end
...
function set_split_drop_position(entity, new_position, new_offset)
  return set_drop_position(
    entity,
    combine_drop_position(new_position, new_offset)
  )
end
...
function set_both_positions(entity, newpickup, newdrop)
  ...
  local new_positions = {
    drop_position = {
      x = entity.position.x + newdrop.x,
      y = entity.position.y + newdrop.y
    },
    pickup_position = {
      x = entity.position.x + newpickup.x,
      y = entity.position.y + newpickup.y
    }
  }
  entity.drop_position = new_positions.drop_position
  entity.pickup_position = new_positions.pickup_position
...
function bobmods.logistics.set_positions(entity, player_index)
  if global.bobmods.logistics[player_index].enabled then
    ...
    if near_unlocked then
      if near then
        drop_offset =
          bobmods.logistics.set_range(drop_position, -bobmods.inserters.offset)
      else
        drop_offset =
          bobmods.logistics.set_range(drop_position, bobmods.inserters.offset)
      end
  ...
  if global.bobmods.logistics[player_index].enabled2 and remote.interfaces.bobinserters then
    local pickup_position =
      remote.call("bobinserters", "get_position", {
        position = global.bobmods.logistics[player_index].pickup
      })
    local drop_position =
      remote.call("bobinserters", "get_position", {
        position = global.bobmods.logistics[player_index].drop
      })
    local drop_offset =
      remote.call("bobinserters", "get_offset", {
        position = global.bobmods.logistics[player_index].offset
      })
    ...
    -- 2 is up, because inserters are backwards.
...
function bobmods.inserters.gui_drop_click(entity, element)
  ...
  local drop_position, drop_offset = get_split_drop_position(entity)
  local new_offset =
    calculate_new_drop_offset(drop_position, drop_offset, new_position)
  set_split_drop_position(entity, new_position, new_offset)
  entity.direction = entity.direction -- set direction to force update
  bobmods.inserters.refresh_position_checkboxes(
    entity,
    game.players[element.player_index]
  )
end
...
function bobmods.inserters.rotate_pickup(entity, player)
  local pickup_position = get_pickup_position(entity)
  pickup_position =
    bobmods.inserters.rotate_position(player.force, pickup_position)
  set_pickup_position(entity, pickup_position)
...
function bobmods.inserters.rotate_drop(entity, player)
  local drop_position, drop_offset = get_split_drop_position(entity)
  local new_position =
    bobmods.inserters.rotate_position(player.force, drop_position)
  local new_offset =
    calculate_new_drop_offset(drop_position, drop_offset, new_position)
  set_split_drop_position(entity, new_position, new_offset)
  if entity == global.bobmods.inserters[player.index].entity then
...
function bobmods.inserters.pickup_range(entity, player)
  local long_unlocked =
    tech_unlocked(player.force, bobmods.inserters.long_technology)
  local long2_unlocked =
    tech_unlocked(player.force, bobmods.inserters.long2_technology)
  local pickup_position = get_pickup_position(entity)
  local islong = false
  local islong2 = false
...
function bobmods.inserters.near_range(entity, player)
  local near_unlocked =
    tech_unlocked(player.force, bobmods.inserters.near_technology)

  local drop_position, drop_offset = get_split_drop_position(entity)

  if near_unlocked then
    if (drop_offset.x == 0 and drop_offset.y == 0) or (drop_offset.x == 0 and drop_position.x ~= 0) or (drop_offset.y == 0 and drop_position.y ~= 0) then
      drop_offset = bobmods.inserters.normalise_tile_offset({
        x = drop_position.x,
        y = drop_position.y
      })
    else
    ...
    if (drop_position.y ~= 0 and drop_position.x ~= 0) and ((((drop_position.x > 0 and drop_offset.x < 0) or (drop_offset.x > 0 and drop_position.x < 0)) and ((drop_position.y > 0 and drop_offset.y > 0) or (drop_offset.y < 0 and drop_position.y < 0))) or (((drop_position.x > 0 and drop_offset.x > 0) or (drop_offset.x < 0 and drop_position.x < 0)) and ((drop_position.y > 0 and drop_offset.y < 0) or (drop_offset.y > 0 and drop_position.y < 0)))) then -- x near -- y far -- x far -- y near
      drop_offset = bobmods.inserters.normalise_tile_offset({
        x = drop_position.x,
        y = drop_position.y
      })
...
function bobmods.logistics.destroy_gui_button(player_index)
  ...
  local flow =
    gui.mod_gui_button_flow or (gui.mod_gui_top_frame and gui.mod_gui_top_frame.mod_gui_inner_frame)
...
function bobmods.logistics.create_gui_button(player_index)
  ...
  if not settings.get_player_settings(
    player
  )["bobmods-inserters-button-enable"].value then return end
  ...
  mod_gui.get_button_flow(player).add{
    ...
    visible = settings.get_player_settings(
      player
    )["bobmods-inserters-button-enable"].value
  }
end
...
function bobmods.logistics.create_gui(player_index)
  ...
  local long_unlocked =
    tech_unlocked(
      game.players[player_index].force,
      bobmods.inserters.long_technology
    )
  local more_unlocked =
    tech_unlocked(
      game.players[player_index].force,
      bobmods.inserters.more_technology
    )
  local near_unlocked =
    tech_unlocked(
      game.players[player_index].force,
  ...
  local gui = player.gui.left.add({
    type = "frame",
    name = "bob_logistics_inserter_gui",
    direction = "vertical"
  })
...
function bobmods.inserters.open_gui(entity, player)
  ...
  local long2_unlocked =
    tech_unlocked(player.force, bobmods.inserters.long2_technology)
  local more_unlocked =
    tech_unlocked(player.force, bobmods.inserters.more_technology)
  local more2_unlocked =
    tech_unlocked(player.force, bobmods.inserters.more2_technology)
  local near_unlocked =
    tech_unlocked(player.force, bobmods.inserters.near_technology)
  ...
  if globtable.position == "left" then
    ...
    gui.anchor = {
      ...
      position = defines.relative_gui_position[settings.get_player_settings(
        player
      )["bobmods-inserters-gui-position"].value]
    }

    globtable.pickup, globtable.drop, globtable.offset =
    ...
    if settings.get_player_settings(
      player
    )["bobmods-inserters-show-window"].value ~= "off" then
      local entity_flow = gui
      local frame_width = 400
      if settings.get_player_settings(
        player
      )["bobmods-inserters-show-window"].value == "left" then
        gui.title_flow.title.caption = {"gui.bob-inserter-gui"}
        main_table_target = gui.add({
          type = "flow",
          name = "main_flow",
          style = "horizontal_flow"
        })
        gui.main_flow.style.horizontal_spacing = 12
        entity_flow = gui.main_flow.add({
          type = "table",
          name = "entity_flow",
          style = "bob_table",
          column_count = 1
...
function bobmods.inserters.draw_offset_grid(gui, position)
  gui.add({
    ...
    name = "button1",
    state = false,
    style = "bob_inserter_checkbox_small"
  })
  gui.add({
    type = "checkbox",
    name = "button2",
...
function bobmods.inserters.draw_button_grid(
gui,
  position,
  style,
  long,
...
function bobmods.inserters.refresh_position_checkboxes(entity, player)
  ...
  if global.bobmods.inserters[player.index].pickup then
    bobmods.inserters.draw_button_position_check(
      global.bobmods.inserters[player.index].pickup,
      pickup_position
    )
  end
  if global.bobmods.inserters[player.index].drop then
    bobmods.inserters.draw_button_position_check(
      global.bobmods.inserters[player.index].drop,
      drop_position
    )
  end
  if global.bobmods.inserters[player.index].offset then
    bobmods.inserters.draw_button_offset_check(
      global.bobmods.inserters[player.index].offset,
      drop_offset
    )
  end
end
...
position: the index in table bobmods.inserters.positions to define the pickup/drop location.
  ...
  draw_button_position_check = function(data)
    bobmods.inserters.draw_button_position_check(
      data.gui,
      bobmods.inserters.positions[data.position]
    )
  end,
  --[[
This function will refresh the GUI checkboxes, designed to refresh a drawn GUI grid after you click one of it's buttons.
...
position: the index in table bobmods.inserters.offset_positions to define the offset location.
  ...
  draw_button_offset_check = function(data)
    bobmods.inserters.draw_button_offset_check(
      data.gui,
      bobmods.inserters.offset_positions[data.position]
    )
  end,
  --[[
