...
data.raw.character.character.inventory_size =
  settings.startup["bobmods-plates-inventorysize"].value

if data.raw["god-controller"] and data.raw["god-controller"]["default"] then
  data.raw["god-controller"]["default"].inventory_size =
    settings.startup["bobmods-plates-inventorysize"].value
end
...
if settings.startup["bobmods-colorupdate"].value == true then
  ...
  data.raw.recipe["battery"].crafting_machine_tint.primary = {
    r = 1.0,
    g = 0.2,
    b = 0.1,
    a = 1.000
  ...
  data.raw.item["lithium-ion-battery"].icon =
    "__bobplates__/graphics/icons/battery-blue.png"
  data.raw.item["lithium-ion-battery"].icon_size = 64
  data.raw.recipe["lithium-ion-battery"].crafting_machine_tint.primary = {
    r = 0.1,
    g = 0.5,
    b = 1.0,
    a = 1.000
  ...
  data.raw.item["silver-zinc-battery"].icon =
    "__bobplates__/graphics/icons/battery-purple.png"
  data.raw.item["silver-zinc-battery"].icon_size = 64
  data.raw.recipe["silver-zinc-battery"].crafting_machine_tint.primary = {
    r = 0.7,
    g = 0.9,
    b = 1.0,
    a = 1.000
...
bobmods.lib.machine.type_if_add_category(
  ...
  "crafting-with-fluid",
  "distillery"
) -- Adds distilling recipies to assembling machines that can handle fluids
bobmods.lib.machine.type_if_add_category(
  "assembling-machine",
  "chemistry",
  "distillery"
) -- Adds distilling recipies to chemical plants
...
if settings.startup["bobmods-plates-batteryupdate"].value == true then
  data.raw.technology["battery"].prerequisites =
    {"sulfur-processing", "plastics"}
  data.raw.recipe["battery"].normal.ingredients = {{"lead-plate", 2}, {
    type = "fluid",
    name = "sulfuric-acid",
    amount = 20
  }, {"plastic-bar", 1}}
  data.raw.recipe["battery"].expensive.ingredients = {{"lead-plate", 2}, {
    type = "fluid",
    name = "sulfuric-acid",
    amount = 40
  }, {"plastic-bar", 2}}
...
--Nuclear fuel update.
data.raw.item["uranium-fuel-cell"].fuel_glow_color = {
  r = 0,
  g = 1,
  b = 0
}
...
if settings.startup["bobmods-plates-nuclearupdate"].value == true then
  ...
  data.raw.recipe["nuclear-fuel-reprocessing"].crafting_machine_tint.secondary
  = {
    r = 1,
    g = 0.7,
    b = 0
  } --Right hand module glows plutonium orange-yellow.
  ...
  bobmods.lib.recipe.add_ingredient("nuclear-fuel-reprocessing", {
    "used-up-uranium-fuel-cell",
    5
  }) -- +5 from base
  bobmods.lib.recipe.set_result("nuclear-fuel-reprocessing", {
    type = "item",
    name = "uranium-238",
    amount = 6,
    catalyst_amount = 5
...
for i, recipe in pairs(data.raw.recipe) do
  if (string.sub(recipe.name, 1, 5) == "fill-" or string.sub(
    recipe.name,
    1,
    6
  ) == "empty-") and recipe.category == "crafting-with-fluid" then
...
if settings.startup["bobmods-plates-vanillabarrelling"].value == true then
  bobmods.lib.machine.type_if_add_category(
    ...
    "crafting-with-fluid",
    "barrelling"
  ) -- Adds barrelling to assembling machines
  bobmods.lib.machine.type_if_add_category(
    "assembling-machine",
    "crafting-with-fluid",
    "air-pump"
...
if settings.startup["bobmods-plates-purewater"].value == true then
  ...
  bobmods.lib.resource.add_result("ground-water", {
    type = "fluid",
    name = "pure-water",
    amount = 10,
    probability = 1
  ...
  bobmods.lib.recipe.add_result("bob-heavy-water", {
    type = "fluid",
    name = "pure-water",
    amount = 99.5
  })
...
data.raw.fluid["petroleum-gas"].flow_color = {
  r = 0.6,
  g = 0.2,
  b = 0.6
}
