if settings.startup["bobmods-power-steam"].value == true then
  ...
  if data.raw.item["copper-tungsten-pipe"] then
    bobmods.lib.recipe.add_ingredient("oil-boiler-4", {
  ...
  if settings.startup["bobmods-plates-convert-recipes"] and settings.startup["bobmods-plates-convert-recipes"].value == true then
    if data.raw.item["steel-pipe"] then
      ...
      bobmods.lib.recipe.add_result("boiler-2-from-oil-boiler", {
        "steel-pipe",
        2
      })
    end
    ...
    if data.raw.item["brass-pipe"] then
      ...
      bobmods.lib.recipe.add_result("boiler-3-from-oil-boiler-2", {
        "brass-pipe",
        2
      })
    end
    ...
    if data.raw.item["ceramic-pipe"] then
      ...
      bobmods.lib.recipe.add_result("boiler-4-from-oil-boiler-3", {
        "ceramic-pipe",
        2
      })
    end
    ...
    if data.raw.item["copper-tungsten-pipe"] then
      ...
      bobmods.lib.recipe.add_result("boiler-5-from-oil-boiler-4", {
        "copper-tungsten-pipe",
        2
      })
    elseif data.raw.item["tungsten-pipe"] then
