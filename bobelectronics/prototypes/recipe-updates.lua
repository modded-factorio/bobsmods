for i, recipe in pairs(data.raw.recipe) do
  if recipe.name == "offshore-pump" or recipe.name == "inserter" or recipe.name == "electric-mining-drill" or recipe.name == "lab" or recipe.name == "radar" or recipe.name == "repair-pack" or recipe.name == "assembling-machine-1" or recipe.name == "splitter" or recipe.name == "small-lamp" or recipe.name == "frame-1" then --DyTech basic mining drill
...
if data.raw.item["silicon-wafer"] then
  bobmods.lib.recipe.add_ingredient("electronic-components", {
    "silicon-wafer",
    2
  })
  bobmods.lib.recipe.add_ingredient("intergrated-electronics", {
    "silicon-wafer",
    4
  })
  bobmods.lib.recipe.add_ingredient("processing-electronics", {
    "silicon-wafer",
    6
  })
else
...
if data.raw.item["gold-plate"] then
  ...
  bobmods.lib.recipe.add_ingredient("multi-layer-circuit-board", {
    "gold-plate",
    2
  })
else
  if data.raw.item["tin-plate"] then
    bobmods.lib.recipe.add_ingredient("superior-circuit-board", {
      "tin-plate",
      1
    })
    bobmods.lib.recipe.add_ingredient("multi-layer-circuit-board", {
